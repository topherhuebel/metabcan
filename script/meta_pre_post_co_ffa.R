...
### read in data
#cholesterol
#phenotype <- read.table("Total_cholesterol.tsv", header = T, sep = "\t")
#dim(phenotype)
...
write.table(meta_analysis, file = paste(phenotype_variable,"_meta_analysis_pretreatment_results_table.tsv", sep = ""), sep = "\t", col.names = T, quote = F)
...
forest(meta_analysis, 
       xlab =paste(phenotype_label, "Pre-Treatment [mg/L]"), 
       digits.mean = 1, 
       digits.sd = 1, 
       comb.fixed = F, 
       #prediction = T,
...
metabias(meta_analysis, method="rank", k.min = 8)
sink()
...
metabias(meta_analysis, method="linreg", k.min = 8, plotit = T)
dev.off()

sink(file = paste(phenotype_variable,"_eggers_test_pretreatment.txt", sep = ""))
metabias(meta_analysis, method="linreg", k.min = 8)
sink()
...
metabias(meta_analysis, method = "mm", k.min = 8)
sink()
...
meta_analysis_meta_regression <- metareg(meta_analysis, meta_data$an_subtype)
...
### calculate meta-analysis for continuous outcome BMI-only
...
bubble(meta_analysis_meta_regression, 
       ...
       xlab = "Mean BMI anorexia nervosa \n cases pre-treatment")
dev.off()


### meta-regression of illness duration
### calculate meta-analysis for continuous outcome with DOD-only
...
write.table(meta_analysis_posttreatment, file = paste(phenotype_variable,"_meta_analysis_posttreatment_results_table.tsv", sep = ""), sep = "\t", col.names = T, quote = F)

#### Forest plot ####
### create forest plot with meta package
png(paste(phenotype_variable,"_forest_plot_posttreatment.png", sep = ""), height = 750, width = 800)
forest(meta_analysis_posttreatment, 
       xlab =paste(phenotype_label, "Post-Treatment [mg/L]"), 
       digits.mean = 1, 
       digits.sd = 1, 
       comb.fixed = F, 
       #       prediction = T,
...
radial(meta_analysis_posttreatment, k.min = 5)
dev.off()
...
write.table(meta_analysis_pre_post, file = paste(phenotype_variable,"_meta_analysis_pre_post_results_table.tsv", sep = ""), sep = "\t", col.names = T, quote = F)

#### Forest plot ####
### create forest plot with meta package
png(paste(phenotype_variable,"_forest_plot_pre_post.png", sep = ""), height = 750, width = 800)
forest(meta_analysis_pre_post, 
       xlab =paste(phenotype_label, "Post-Treatment [mg/dL]"), 
       digits.mean = 1, 
       digits.sd = 1, 
       comb.fixed = F, 
       #       prediction = T,
...
bubble(meta_analysis_meta_regression, 
       ylab = paste("Difference in", phenotype_label, "Pre-Post-Treatment [mg/dL]"),
